...
actor Main is TestList
  ...
  fun tag tests(test: PonyTest) =>
    ...
    test(_TestPrintOrderedObject)
    test(_TestPrintSSBObject)
...
class iso _TestPrintOrderedObject is UnitTest
  fun name(): String => "JSON/print.ordered_object"

  fun apply(h: TestHelper) =>
    let doc: JsonDoc = JsonDoc
    let obj: JsonObject = JsonObject
    let comp = {(s: String): USize => s.size()} val

    doc.data = obj
    h.assert_eq[String]("{}", doc.string("  ", true, comp))

    obj.data.clear()
    obj.data("a") = true
    obj.data("aa") = true
    obj.data("aaa") = true
    let s = doc.string("  ", true, comp)
    h.assert_eq[String]("{\n  \"a\": true,\n  \"aa\": true,\n  \"aaa\": true\n}", s)

class iso _TestPrintSSBObject is UnitTest
  fun name(): String => "JSON/print.ssb_object"

  fun apply(h: TestHelper) =>
    let doc: JsonDoc = JsonDoc
    let obj: JsonObject = JsonObject
    let comp = {(s: String): USize => match s
      | "previous" => 0
      | "author" => 1
      | "sequence" => 2
      | "timestamp" => 3
      ...
      | "content" => 5
      | "signature" => 6
      else USize.max_value() end} val

    doc.data = obj

    obj.data("previous") = ""
    obj.data("author") = ""
    obj.data("sequence") = I64(0)
    obj.data("timestamp") = I64(0)
    obj.data("content") = ""
    obj.data("signature") = ""
    obj.data("hash") = "sha256"
    let s = doc.string("  ", true, comp)
    let repr = "{\n  \"previous\": \"\",\n  \"author\": \"\",\n  \"sequence\": 0,\n  \"timestamp\": 0,\n  \"hash\": \"sha256\",\n  \"content\": \"\",\n  \"signature\": \"\"\n}"
    h.assert_eq[String](repr, s)
